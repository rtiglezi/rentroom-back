...
test('get /users/:id', () => {
    return request(address)
        ...
        .then(response => {
            expect(response.status).toBe(200)
            expect(response.body.name).toBe('usuario 4')
            expect(response.body.email).toBe('user4@email.com')
...
test('patch /users/:id', () => {
    return request(address)
        ...
        .then(response => {
            expect(response.status).toBe(200)
            expect(response.body._id).toBeDefined()
            expect(response.body.name).toBe('usuario7 - patch')
            expect(response.body.email).toBe('usuario7@email.com')
            expect(response.body.password).toBeUndefined()
        })
        .catch(fail)
})
...
test('put /users:/id', () => {
    return request(address)
        ...
        .then(response => {
            expect(response.status).toBe(200)
            expect(response.body.name).toBe('usuario 8')
            expect(response.body.email).toBe('usuario8@email.com')
